...
export default class RoofsLayer extends CanvasLayer {
  ...
  static getOpacity(tile) {
    const open =
      tile.getFlag("roofs", "open") ??
      game.settings.get("roofs", "defaultOpen");
    const closed =
      tile.getFlag("roofs", "closed") ??
      game.settings.get("roofs", "defaultClosed");
  ...
  static _sightUpdate() {
    ...
    canvas.tiles.placeables.forEach((tile) => {
      ...
      canvas.tokens.placeables
        .filter(
          (token) =>
            !token.observer && token.visible && RoofsLayer.inBounds(tile, token)
        )
        .forEach((token) => {
          token.visible = false;
        });
  ...
  static _patchDrag() {
    ...
    Tile.prototype._onDragLeftStart = function _onDragLeftStart(event) {
      const targets = this.layer.options.controllableObjects
        ? this.layer.controlled
        : [this];
  ...
  static async extendTileHUD(hud, html) {
    ...
    // Append template
    const form = await renderTemplate("/modules/roofs/templates/hud.hbs", {
      isRoof,
      open,
      closed,
      mode,
